...
Can call XNeroWebVR's VR functionality from other folders and read/print VR control information
"""
...
# Set the absolute path to the xnero_webvr folder
XNERO_WEBVR_PATH = "/home/loopkok/loopkok/xnero_webvr"

def setup_xnero_webvr_environment():
    """Setup xnero_webvr environment"""
    # Add xnero_webvr path to Python path
    if XNERO_WEBVR_PATH not in sys.path:
        sys.path.insert(0, XNERO_WEBVR_PATH)
    
    # Set working directory
    os.chdir(XNERO_WEBVR_PATH)
    
    # Set environment variables
    os.environ['PYTHONPATH'] = f"{XNERO_WEBVR_PATH}:{os.environ.get('PYTHONPATH', '')}"
...
def import_xnero_webvr_modules():
    """Import xnero_webvr modules"""
    try:
        from xnero_webvr.config import XNeroWebVRConfig
        from xnero_webvr.inputs.vr_ws_server import VRWebSocketServer
        from xnero_webvr.inputs.base import ControlGoal, ControlMode
        return XNeroWebVRConfig, VRWebSocketServer, ControlGoal, ControlMode
    except ImportError as e:
        print(f"Error importing xnero_webvr modules: {e}")
        print(f"Make sure XNERO_WEBVR_PATH is correct: {XNERO_WEBVR_PATH}")
...
class SimpleAPIHandler(http.server.BaseHTTPRequestHandler):
    ...
    def serve_file(self, filename, content_type):
        ...
        try:
            ...
            web_root = getattr(self.server, 'web_root_path', XNERO_WEBVR_PATH)
...
class SimpleHTTPSServer:
    ...
    def __init__(self, config):
        ...
        self.web_root_path = XNERO_WEBVR_PATH
...
class VRMonitor:
    ...
    def __init__(self):
        ...
        self.left_hand_data = None   # Hand tracking joint data for left hand
        self.right_hand_data = None  # Hand tracking joint data for right hand
        self._goal_lock = threading.Lock()  # Add thread lock
    
    def initialize(self):
        """Initialize VR monitor"""
        print("🔧 Initializing XNeroWebVR Monitor...")
        
        # Setup environment
        setup_xnero_webvr_environment()
        
        # Import modules
        XNeroWebVRConfig, VRWebSocketServer, ControlGoal, ControlMode = import_xnero_webvr_modules()
        if XNeroWebVRConfig is None:
            print("❌ Failed to import xnero_webvr modules")
            return False
        
        # Create configuration
        self.config = XNeroWebVRConfig()
        ...
        try:
            self.vr_server = VRWebSocketServer(
                ...
                input_mode=self.config.vr_input_mode
            )
        except Exception as e:
        ...
        print("✅ XNeroWebVR Monitor initialized successfully")
        ...
        while self.is_running:
            try:
                ...
                with self._goal_lock:
                    if goal.arm == "left":
                        self.left_goal = goal
                        # Extract hand data from metadata if present
                        if goal.metadata and 'hand_data' in goal.metadata:
                            self.left_hand_data = goal.metadata['hand_data']
                    elif goal.arm == "right":
                        self.right_goal = goal
                        # Extract hand data from metadata if present
                        if goal.metadata and 'hand_data' in goal.metadata:
                            self.right_hand_data = goal.metadata['hand_data']
    ...
    def get_left_hand_nowait(self):
        """Return the latest left hand data in MediaPipe 21-landmark format, or None.

        Returns dict with keys:
          - hand: "left"
          - landmarks: array of 21 {x, y, z} in MediaPipe order (0=WRIST, 4=THUMB_TIP, 8=INDEX_TIP, ...)
          - wrist: {position, quaternion, radius}  (6DoF wrist pose for IK control)
        """
        with self._goal_lock:
            return self.left_hand_data

    def get_right_hand_nowait(self):
        """Return the latest right hand joint data if available, else None."""
        with self._goal_lock:
            return self.right_hand_data

    def get_hand_data_nowait(self):
        """Return dict with both left and right hand joint data."""
        with self._goal_lock:
            return {
                "left": self.left_hand_data,
                "right": self.right_hand_data,
                "has_left": self.left_hand_data is not None,
                "has_right": self.right_hand_data is not None
            }
...
def main():
    """Main function"""
    print("🎮 XNeroWebVR Monitor - XNeroWebVR VR Control Information Monitor")
    print("=" * 60)
    
    # Check XNeroWebVR path
    if not os.path.exists(XNERO_WEBVR_PATH):
        print(f"❌ XNeroWebVR path does not exist: {XNERO_WEBVR_PATH}")
        print("Please update XNERO_WEBVR_PATH in the script")
        return
    ...
    try:
        ...
        print("\n👋 XNeroWebVR Monitor stopped by user")
    except Exception as e:
