...
from ..config import XNeroWebVRConfig
...
class VRWebSocketServer(BaseInputProvider):
    ...
    def __init__(self, command_queue: asyncio.Queue, config: XNeroWebVRConfig, print_only: bool = False, input_mode: str = "controller"):
        super().__init__(command_queue)
        self.config = config
        self.clients: Set = set()
        self.server = None
        self.print_only = print_only  # New flag for print-only mode
        self.input_mode = input_mode  # "controller" | "hand" | "both"

        # Controller states
        self.left_controller = VRControllerState("left")
        self.right_controller = VRControllerState("right")

        # Hand tracking state (wrist-as-controller in hand/both modes)
        self.left_hand_state = VRControllerState("left")
        self.right_hand_state = VRControllerState("right")
    ...
    async def start(self):
        ...
        mode_labels = {"controller": "🎮 Controller", "hand": "🖐️  Hand Tracking", "both": "🎮🖐️ Controller + Hand Tracking"}
        print(f"[VR_WS] Input mode: {mode_labels.get(self.input_mode, self.input_mode)}")
    ...
    async def process_controller_data(self, data: Dict):
        ...
        # Suppress per-frame debug; hand/controller activity logged only on change

        # 检查是否有摇杆或按钮操作，只在有操作时打印
        # 只在 controller 或 both 模式下才检测手柄按键，hand 模式下跳过
        has_thumbstick_or_button_activity = False
        thumbstick_info = []
        button_info = []

        # 检查左右手柄的摇杆和按钮状态（仅在 controller/both 模式）
        if self.input_mode in ("controller", "both"):
        ...
        # Process controller data (only in controller or both modes)
        if self.input_mode in ("controller", "both"):
        ...
        # Process hand tracking data (only in hand or both modes)
        if self.input_mode in ("hand", "both"):
            if 'leftHand' in data and data['leftHand']:
                await self.process_hand_data('left', data['leftHand'])
            if 'rightHand' in data and data['rightHand']:
                await self.process_hand_data('right', data['rightHand'])
    ...
    async def process_single_controller(self, hand: str, data: Dict):
        ...
        if position and all(k in position for k in ['x', 'y', 'z']):
            ...
            # 创建绝对位置控制目标
            is_hand_source = data.get('_hand_data') is not None
            metadata = {
                "source": "vr_hand_tracking" if is_hand_source else "vr_absolute_position",
            ...
            # Attach full hand joint data if available (from hand tracking mode)
            if is_hand_source:
                metadata["hand_data"] = data['_hand_data']
    ...
    async def process_hand_data(self, hand: str, hand_data: Dict):
        """Process WebXR hand tracking data for a single hand.

        Uses the wrist joint 6DoF pose as a direct replacement for controller
        pose, feeding it into the existing controller processing pipeline.
        The full MediaPipe-format 21-landmark data is attached to metadata for recording.
        """
        landmarks = hand_data.get('landmarks', [])
        wrist = hand_data.get('wrist')
        # Log once silently confirmed OK
        hand_key = f"_logged_{hand}"
        if not getattr(self, hand_key, False):
            setattr(self, hand_key, True)
            print(f"[VR_WS] Hand {hand.upper()} tracking active: {len(landmarks)} landmarks, wrist={'OK' if wrist else 'MISSING'}")
        if wrist is None:
            return

        wrist_pos = wrist.get('position', {})
        wrist_quat = wrist.get('quaternion', {})

        if not wrist_pos:
            return

        # Build a synthetic controller data dict from the wrist joint.
        # This reuses process_single_controller() unchanged.
        synthetic_ctrl = {
            'position': wrist_pos,
            'quaternion': wrist_quat,
            'rotation': {},
            'gripActive': True,    # hand tracking is always "active"
            'trigger': 0,          # finger curl will be computed separately
            '_hand_data': hand_data  # carry full hand joint data
        }
        await self.process_single_controller(hand, synthetic_ctrl)
